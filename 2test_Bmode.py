...
os.system('rm -rf /smc/jianyao/datasets/image_matrix/2_channels/test; mkdir /smc/jianyao/datasets/image_matrix/2_channels/test')
import itchat
import threading
...
#		cmb_map = np.random.uniform(-600,700,12*nside**2)
		...
		for fre in range(7):
		    ...
		    with file('/smc/jianyao/datasets/image_matrix/2_channels/test/QU_%s_%d.txt'%(nu[fre], k), 'w') as outfile:
...
os.system(' python ../test.py --dataroot /smc/jianyao/datasets/image_matrix/2_channels --name /smc/jianyao/checkpoints/2_channels --model pix2pix --which_model_netG unet_256 --which_direction BtoA --dataset_mode aligned --norm batch --input_nc 2 --output_nc 2')
