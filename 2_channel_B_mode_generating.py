...
#		cmb_map = np.random.uniform(-600,700,12*nside**2)
		...
		for fre in range(7):
		    ...
		    with file('/smc/jianyao/datasets/image_matrix/2_channels/train/QU_%s_%d.txt'%(nu[fre], k), 'w') as outfile:
...
os.system('nohup python ../train.py --dataroot /smc/jianyao/datasets/image_matrix/2_channels --name /smc/jianyao/checkpoints/2_channels --model pix2pix --which_model_netG unet_256 --which_direction BtoA --dataset_mode aligned --no_lsgan --norm batch --pool_size 0 --loadSize 256 --fineSize 256 --gpu_ids 0,1 --input_nc 2 --output_nc 2 --display_freq 1 --batchSize 32 --save_epoch_freq 100 --display_id 0 --niter 300 --niter_decay 300 &')
