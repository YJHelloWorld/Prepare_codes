...
######n  null_tes
		cmb_map_real = hp.synfast(cl_real, nside, new = True)
		cmb_map = np.zeros_like(cmb_map_real)
...
####
		with file('/smc/jianyao/datasets/image_matrix/null_test/test/QU_7_fre_%s.txt'%(k), 'w') as outfile:
...
os.system('python ../test.py --dataroot /smc/jianyao/datasets/image_matrix/null_test/ --name /smc/jianyao/checkpoints/noise_ali --model pix2pix --which_model_netG unet_256 --which_direction BtoA --dataset_mode aligned --norm batch --input_nc 6 --output_nc 2')
np.savetxt('/smc/jianyao/checkpoints/noise_ali/norm_factors.txt', Fg)
