...
#os.system('rm -rf /smc/jianyao/checkpoint; mkdir /smc/jianyao/datasets/image_matrix/2_channels/train')
import itchat
import threading
...
#		cmb_map = np.random.uniform(-600,700,12*nside**2)
		...
		with file('/smc/jianyao/datasets/image_matrix/4_fres/test/QU_7_fre_%s.txt'%(k), 'w') as outfile:
...
#			    np.savetxt(outfile, combine_QU[1], fmt = '%.4f')
#			outfile.write('#New dimension\n')
#		    np.savetxt(outfile, combine_QU[0])
#		    np.savetxt(outfile, combine_QU[1])
...
#os.system('python ../test.py --dataroot /smc/jianyao/datasets/image_matrix/sync_dust --name /smc/jianyao/checkpoints/sync_dust --model pix2pix --which_model_netG unet_256 --which_direction BtoA --dataset_mode aligned --norm batch --input_nc 6 --output_nc 2')
