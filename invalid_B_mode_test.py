...
Nf = 7 ; k=0; coeff = 1; pp = 2 #polarization 
names = ['intensity', 'Q_map', 'U_map']
cl_QQ = hp.anafast(cmb[1])
cl_UU = hp.anafast(cmb[2])
...
#		cmb_map = np.random.uniform(-600,700,12*nside**2)
	 	k += 1
		with file('/smc/jianyao/datasets/image_matrix/Q_maps/test/%s_%d.txt'%(names[1], k), 'w') as outfile:
		    		
		    cmb_matrix = hp.synfast(cl_QQ, nside)
		    for j in range(7):
	#                 nl = np.ones_like(cl); nl[:] = Nl[j]
	#                 noise = hp.synfast(nl,64)
	#                 
		        total_matrix = total[j][1][pixels] + cmb_matrix[pixels]
		        combine = np.column_stack((cmb_matrix[pixels], total_matrix))
	...
	#                np.savetxt('/home/jianyao/pytorch-CycleGAN-and-pix2pix/datasets/image_matrix/train/totals%s.txt'%(i+687),combine, fmt = '%.2f')
		...
		with file('/smc/jianyao/datasets/image_matrix/U_maps/test/%s_%d.txt'%(names[2], k), 'w') as outfile:

                    cmb_matrix = hp.synfast(cl_UU, nside)
                    for j in range(7):
        #                 nl = np.ones_like(cl); nl[:] = Nl[j]
        #                 noise = hp.synfast(nl,64)
        #                 
                        total_matrix = total[j][2][pixels] + cmb_matrix[pixels]
                        combine = np.column_stack((cmb_matrix[pixels], total_matrix))
                        outfile.write('#Array shape:{0}\n'.format(combine.shape))
        #                np.savetxt('/home/jianyao/pytorch-CycleGAN-and-pix2pix/datasets/image_matrix/train/totals%s.txt'%(i+687),combine, fmt = '%.2f')
                        np.savetxt(outfile, combine, fmt = '%.4f')
                        outfile.write('#New dimension\n')
os.system(' python ../test.py --dataroot /smc/jianyao/datasets/image_matrix/Q_maps --name /smc/jianyao/checkpoints/B_mode --model pix2pix --which_model_netG unet_256 --which_direction BtoA --dataset_mode aligned --norm batch --input_nc 7 --output_nc 1')
os.system(' python ../test.py --dataroot /smc/jianyao/datasets/image_matrix/U_maps --name /smc/jianyao/checkpoints/B_mode --model pix2pix --which_model_netG unet_256 --which_direction BtoA --dataset_mode aligned --norm batch --input_nc 7 --output_nc 1')
