...
Fg = [] # total norm_factor collection, including different k, i.e., different 'fq'...... 
...
#               mean_Q = np.mean(total[NTF][1]+cmb[1]); mean_U = np.mean(total[NTF][2]+cmb[2])
                std_Q = np.std(total[NTF][1]+cmb[1]); std_U = np.std(total[NTF][2]+cmb[2])

		fnf = np.sqrt(std_Q**2 + std_U**2)  #fnf: fiducial norm factor
	###	Q_U = [0, std_Q, std_U]; Q_U_mean = [0, mean_Q, mean_U]
		fg = []; fg.append(fnf)   # factor group, to collect the factors of each frequencies to recover the original matrix before normalization. 
####
		with file('/smc/jianyao/datasets/image_matrix/2_fres_norm_new_with_norm_factors/test/QU_7_fre_%s.txt'%(k), 'w') as outfile:
			
		    for fre in range(Nf):
			
			total_with_cmb_Q = cmb_map[1] + total[fre][1]
			total_with_cmb_U = cmb_map[2] + total[fre][2]
	...
	#              
			    ...
			    cmb_matrix = cmb_norm[pixels]
                            total_norm = (cmb_map[j]+ total[fre][j])/nf*fnf ##- np.mean(total_with_cmb))/np.std(total_with_cmb)*Q_U[j]
...
####
	...
	#                np.savetxt('/home/jianyao/pytorch-CycleGAN-and-pix2pix/datasets/image_matrix/train/totals%s.txt'%(i+687),combine, fmt = '%.2f')
		    ...
		    for j in range(1,3):
                    	...
                    	cmb_matrix = cmb_norm[pixels]
                    	total_norm = (total[NTF][j] +cmb_map[j])# - Q_U_mean[j])/Q_U[j]
                    	total_matrix = total_norm[pixels]
			combine = np.column_stack((cmb_matrix, total_matrix))
                        outfile.write('#Array shape:{0}\n'.format(combine.shape))
        #                np.savetxt('/home/jianyao/pytorch-CycleGAN-and-pix2pix/datasets/image_matrix/train/totals%s.txt'%(i+687),combine, fmt = '%.2f')
                        ...
                        outfile.write('#New dimension\n')
		Fg.append(fg)
np.savetxt('/smc/jianyao/checkpoints/2_fres_norm_new/norm_factors.txt', Fg)
'''					
...
#os.system('nohup python ../train.py --dataroot /smc/jianyao/datasets/image_matrix/3_fres_norm --name /smc/jianyao/checkpoints/3_fres_norm --model pix2pix --which_model_netG unet_256 --which_direction BtoA --dataset_mode aligned --no_lsgan --norm batch --pool_size 0 --loadSize 256 --fineSize 256 --gpu_ids 0,1 --input_nc 6 --output_nc 2 --display_freq 1 --batchSize 32 --save_epoch_freq 100 --display_id 0 --niter 300 --niter_decay 300 &')
