...
Fg = []; loc = '353_norm_EB'; image_dir = '/smc/jianyao/datasets/image_matrix/%s'%loc; train_dir = os.path.join(image_dir, 'train'); test_dir = os.path.join(image_dir, 'test')  # collection for fg...
...
####
		with file('/smc/jianyao/datasets/image_matrix/%s/test/QU_7_fre_%s.txt'%(loc, k), 'w') as outfile:
...
####
	...
	#                np.savetxt('/home/jianyao/pytorch-CycleGAN-and-pix2pix/datasets/image_matrix/train/totals%s.txt'%(i+687),combine, fmt = '%.2f')
        ...
        #                np.savetxt('/home/jianyao/pytorch-CycleGAN-and-pix2pix/datasets/image_matrix/train/totals%s.txt'%(i+687),combine, fmt = '%.2f')
                        ...
                        outfile.write('#New dimension\n')
		Fg.append(fg)
np.savetxt('/smc/jianyao/checkpoints/%s/norm_factors.txt'%(loc), Fg)
'''				
...
os.system('python ../test.py --dataroot /smc/jianyao/datasets/image_matrix/353_norm_EB/ --name /smc/jianyao/checkpoints/353_norm_EB --model pix2pix --which_model_netG unet_256 --which_direction BtoA --dataset_mode aligned --norm batch --input_nc 6 --output_nc 2')
os.system('mv /smc/jianyao/checkpoints/353_norm_EB/norm_factors.txt /smc/jianyao/checkpoints/353_norm_EB/test_latest')
