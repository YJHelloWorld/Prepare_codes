...
#		cmb_map = np.random.uniform(-600,700,12*nside**2)
		cmb_matrix = cmb_map[pixels]
	 	k += 1
		print k
		with file('/smc/jianyao/datasets/image_matrix/train_for_test/totals_100_5000_%s.txt'%(k), 'w') as outfile:
...
os.system('nohup python ../train.py --dataroot /smc/jianyao/datasets/image_matrix --name /smc/jianyao/checkpoints/random_and_normal_cl_0713 --model pix2pix --which_model_netG unet_256 --which_direction BtoA --dataset_mode aligned --no_lsgan --norm batch --pool_size 0 --loadSize 256 --fineSize 256 --gpu_ids 0,1 --input_nc 7 --output_nc 1 --display_freq 1 --batchSize 32 --save_epoch_freq 100 --display_id 0 --niter 300 --niter_decay 300 --continue_train &')
