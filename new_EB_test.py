...
mask[pixel_phi<np.pi*5/3]=1;mask[pixel_phi<np.pi/3] = 0
mask[pixel_theta > np.radians(85)] = 1; mask[pixel_theta < np.radians(5)] = 1
b = np.arange(len(mask)); c = b[mask==False]; pixels = np.append(c, np.zeros(5)).reshape(43,43).astype('int')	
...
###
		NTF = 0    ###   normalize to which frequency, change from 353 to 30.
...
####
		with file('/smc/jianyao/datasets/image_matrix/new_EB_more_data/test/QU_7_fre_%s.txt'%(k), 'w') as outfile:
		    for j in range(1,3):
			cmb_map_EB = hp.alm2map(alms[j],nside)
			cmb_norm = cmb_map_EB#-Q_U_mean[j])/Q_U[j]
                        cmb_matrix = np.pad(cmb_norm[pixels], ((0,1),(0,1)), 'constant')  ## to reshape the (43,43) to (44, 44), padded with 0.
                        total_norm = (total[0][j] +cmb_map[j] + white_noise[0])# - Q_U_mean[j])/Q_U[j]
                        total_matrix = np.pad(total_norm[pixels], ((0,1),(0,1)), 'constant')
		    ...
		    for fre in range(1,Nf):
			    ...
			    cmb_norm = cmb_map_EB/nf*fnf ##-np.mean(total_with_cmb))/np.std(total_with_cmb)*Q_U[j]
                            cmb_matrix = np.pad(cmb_norm[pixels], ((0,1),(0,1)), 'constant')
...
#os.system('nohup python ../test.py --dataroot /smc/jianyao/datasets/image_matrix/new_EB --name /smc/jianyao/checkpoints/new_EB_more_data --model pix2pix --which_model_netG resnet_9blocks --which_direction BtoA --dataset_mode aligned --no_lsgan --norm batch --pool_size 0 --loadSize 44 --fineSize 44 --gpu_ids 1 --input_nc 6 --output_nc 2 --display_freq 1 --batchSize 32 --save_epoch_freq 100 --display_id 0 --niter 300 --niter_decay 300 &')
