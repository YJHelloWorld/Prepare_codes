...
#		cmb_map = np.random.uniform(-600,700,12*nside**2)
	 	k += 1
		print k
		with file('/smc/jianyao/datasets/image_matrix/test/%s_%d.txt'%(names[1], k), 'w') as outfile:
	...
	#                np.savetxt('/home/jianyao/pytorch-CycleGAN-and-pix2pix/datasets/image_matrix/train/totals%s.txt'%(i+687),combine, fmt = '%.2f')
		...
		with file('/smc/jianyao/datasets/image_matrix/test/%s_%d.txt'%(names[2], k), 'w') as outfile:
